...
async fn authenticate(client:        &Client, 
                      client_id:     &str,
                      client_secret: &str) -> Result<AuthResponse> {
    ...
    let response = client.post(OAUTH_URL)
        .headers(headers)
        .body(encoded_body)
        .send().await?;
    if !response.status().is_success() {
        error!("Error: unable to authenticate {}", response.status());
        return Err(anyhow::anyhow!("Error: unable to authenticate {}", response.status()));
    } 
...
impl AuthenticatedClient {
    ...
    pub async fn from_secret(id: &str, secret: &str) -> Result<Self> {
        ...
        let token = authenticate(&client, id, secret).await
    ...
    pub async fn post_json_request(&self, entpoint: &str, body: &str) -> Result<reqwest::Response> {
        ...
        let request =  self.client.post(url).headers(headers).body(body.to_string());
        ...
        let filters = if pq.start_year.is_some() && pq.end_year.is_some() {
            Some(vec![
                 Filter {
                    ...
                    facette: FilterType::SignatureDate,
                 }
            ])
        } else {
            None
...
///
pub async fn get_search_result(aclient: &AuthenticatedClient, pq: &PageQuery)
...
///
pub async fn get_full_text(aclient: &AuthenticatedClient,
                           cid: &str,
                           fond: Fond) -> Result<String> {

    let endpoint = "/consult/getArticle";

    info!("Getting full document for {} in {} using {}", cid, fond, endpoint);

    let data = &json!({
        "id":      cid,
    });

    let response = aclient.post_json_request(endpoint, &serde_json::to_string(data)?).await?;

    if response.status().is_success() {
        let text = response.text().await?;
        let parsed: serde_json::Value = serde_json::from_str(&text)?;
        let out = parsed["article"]["texte"].as_str().unwrap_or("").to_string();
        Ok(out)
    } else {
        let status = response.status();
        let text = response.text().await?;
        ...
        error!("Response: {:?}", text);
        Err(anyhow::anyhow!("Error: unable to get full document {}", status))
    }
}
